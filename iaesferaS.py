...
# Generar una tercera esfera atravesada
u3 = np.linspace(0, 2 * np.pi, 21)
v3 = np.linspace(0, np.pi, 31)
x3 = np.outer(np.cos(u3), np.sin(v3))
y3 = np.outer(np.sin(u3), np.sin(v3))
z3 = np.outer(np.ones(np.size(u3)), np.cos(v3))
z3 = z3 - 2

# Generar una cuarta esfera atravesada
u4 = np.linspace(0, 2 * np.pi, 21)
v4 = np.linspace(0, np.pi, 31)
x4 = np.outer(np.cos(u4), np.sin(v4))
y4 = np.outer(np.sin(u4), np.sin(v4))
z4 = np.outer(np.ones(np.size(u4)), np.cos(v4))
z4 = z4 - 3
...
nodos3 = np.array([x3.flatten(), y3.flatten(), z3.flatten()]).T
nodos4 = np.array([x4.flatten(), y4.flatten(), z4.flatten()]).T

# Crear una malla para conectar los nodos
nodos_combinados = np.concatenate((nodos, nodos2, nodos3, nodos4))
...
ax.scatter(nodos3[:, 0], nodos3[:, 1], nodos3[:, 2], color='green')
ax.scatter(nodos4[:, 0], nodos4[:, 1], nodos4[:, 2], color='yellow')
